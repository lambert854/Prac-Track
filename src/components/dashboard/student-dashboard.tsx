...
import { ArchiveBoxIcon, CheckCircleIcon, ClockIcon, DocumentTextIcon, ExclamationTriangleIcon, PlusIcon } from '@heroicons/react/24/outline'
        ...
        {placement ? (
          <div className="space-y-6">
            ...
            {(placement.status === 'ACTIVE' || placement.status === 'APPROVED_PENDING_CHECKLIST') && 
              (!placement.learningContract || !placement.checklist) && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="flex items-start">
                    <ExclamationTriangleIcon className="h-6 w-6 text-yellow-600 mt-1 mr-3 flex-shrink-0" />
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-yellow-800 mb-2">Missing Required Documents</h3>
                      <p className="text-sm text-yellow-700 mb-3">
                        You still need to upload the following documents to complete your placement application:
                      </p>
                      <ul className="list-disc list-inside space-y-1 text-sm text-yellow-700 mb-4">
                        {!placement.learningContract && (
                          <li>Agency Learning Contract</li>
                        )}
                        {!placement.checklist && (
